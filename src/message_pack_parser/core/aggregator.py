...
def perform_aggregations(
    ...
    if not stats_to_compute:
        stats_to_compute = [name for name, stat in STATS_REGISTRY.items() if stat.default_enabled]
        logger.warning(f"No specific stats requested. Computing default set: {stats_to_compute}")
        ...
        if stat_name in STATS_REGISTRY:
            ...
            try:
                ...
                if result_df is not None and not result_df.is_empty():
                    ...
                    logger.warning(f"Stat '{stat_name}' produced an empty or null DataFrame.")
            except Exception as e:
                logger.error(f"Error calculating stat '{stat_name}': {e}", exc_info=True)
        else:
            logger.warning(f"Requested stat '{stat_name}' is not in STATS_REGISTRY. Skipping.")

    unaggregated_df = UNAGGREGATED_STREAM_REGISTRY['command_log'](dataframes_by_aspect)
    
    logger.info(f"Aggregation complete. Computed {len(computed_stats)} aggregated stats.")
    return computed_stats, unaggregated_df